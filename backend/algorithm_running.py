...
class RankingCalculator:
    def __init__(self, alg_name) -> None:
        ...
        self.rand_seed = 999
        self.runs = 10
        self.results_file = f'algorithm_results_{alg_name}.json'
    
    def run_algorithm(self):
        # with open(f'cec_data_{alg_name}.json') as f:
        #     data = json.load(f)
        data = {}
            ...
            for fun_idx, fun in enumerate(self.functions):
                ...
                for r in range(self.runs):
                    ...
                    try:
                        ...
                        if self.alg_name not in data:
                            data[self.alg_name] = {}
                        if f"function_{fun}" not in data[self.alg_name]:
                            data[self.alg_name][f"function_{fun}"] = {}
                        if f"dim_{dim}" not in data[self.alg_name][f"function_{fun}"]:
                            data[self.alg_name][f"function_{fun}"][f"dim_{dim}"] = {}
                        data[self.alg_name][f"function_{fun}"][f"dim_{dim}"][f"trial_{r}"] = [error, calls_count]
                
                    self.save_progress_to_file(dim_idx, fun_idx, r, max_fes)

                with open(f'cec_data_{alg_name}.json', 'w') as file:
