...
class CECfunctions:
  ...
  def rosenbrock_func(self, x):
    ...
    f = 0.0
    z = self.sr_func(x, self.nx, self.OShift, self.M, 2.048/100.0, self.s_flag, self.r_flag)
    z[0] += 1.0
  ...
  def griewank_func(self, x, nx, Os, Mr, s_flag, r_flag):
    ...
    z = self.sr_func(self, x, nx, Os, Mr, 600.0/100.0, s_flag, r_flag)
    for i in range(nx):
      s += z[i]*z[i]
  ...
  @count_calls
  def schaffer_F7_func(self, x):
    
    f = 0.0
    z = self.sr_func(x, self.nx, self.OShift, self.M, 1.0, self.s_flag, self.r_flag)
    for i in range(self.nx -1):
      z[i]=pow(y[i]*y[i]+y[i+1]*y[i+1],0.5)
      tmp=np.sin(50.0*pow(z[i],0.2))
      f += pow(z[i],0.5)+pow(z[i],0.5)*tmp*tmp 
    f = f*f/(self.nx-1)/(self.nx-1)
    return f

  @count_calls
  def step_rastrigin_func(self, x):
    
    f = 0.0
    for i in range(self.nx):
      if (np.fabs(y[i]-self.OShift[i])>0.5):
        y[i]=self.OShift[i]+np.floor(2*(y[i]-self.OShift[i])+0.5)/2

    z = self.sr_func(x, self.nx, self.OShift, self.M, 5.12/100.0, self.s_flag, self.r_flag)
  ...
  def levy_func(self, x):
    ...
    term3 = pow((w[self.nx-1]-1),2) * (1+pow((np.sin(2*PI*w[self.nx-1])),2))
  ...
  def hf02(self, x):
    ...
    for i in range(cf_num-1):
      G_nx[i] = np.ceil(Gp[i]*self.nx)
      tmp += G_nx[i]
    G_nx[cf_num-1]=self.nx-tmp
    G_nx = np.int64(G_nx)
    G[0]=0
    ...
    for i in range(self.nx):
      y[i]=z[S[i]-1]
    i=0
    fit[i] = self.bent_cigar_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=1
    fit[i] = self.hgbat_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=2
    fit[i] = self.rastrigin_func(y[G[i]:self.nx],G_nx[i],self.OShift,self.M,0,0)
  ...
  def hf10(self, x):
    ...
    fit[i] = self.hgbat_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=1
    fit[i] = self.katsuura_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=2
    fit[i] = self.ackley_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=3
    fit[i] = self.rastrigin_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=4
    fit[i] = self.schwefel_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=5
    fit[i] = self.schaffer_F7_func(y[G[i]:self.nx],G_nx[i],self.OShift,self.M,0,0)
  ...
  def hf06(self, x):
    ...
    fit[i] = self.katsuura_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=1
    fit[i] = self.happycat_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=2
    fit[i] = self.grie_rosen_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=3
    fit[i] = self.schwefel_func(y[G[i]:G[i+1]],G_nx[i],self.OShift,self.M,0,0)
    i=4
    fit[i] = self.ackley_func(y[G[i]:self.nx],G_nx[i],self.OShift,self.M,0,0)
    f = 0.0
    for i in range(cf_num):
      f += fit[i]
    return f
  ...
  def cf01(self, x):
    ...
    i=0
    fit[i] = self.rosenbrock_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    fit[i]=10000*fit[i]/1e+4
    i=1
    fit[i] = self.ellips_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    fit[i]=10000*fit[i]/1e+10
    i=2
    fit[i] = self.bent_cigar_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    fit[i]=10000*fit[i]/1e+30
    i=3
    fit[i] = self.discus_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    fit[i]=10000*fit[i]/1e+10
    i=4
    fit[i] = self.ellips_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,0)
    fit[i]=10000*fit[i]/1e+10
    f = self.cf_cal(x, self.nx, self.OShift, delta,bias,fit,cf_num)
    return f
  ...
  def cf02(self, x):
    ...
    i=0
    fit[i] = self.schwefel_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,0)
    i=1
    fit[i] = self.rastrigin_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    i=2
    fit[i] = self.hgbat_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    f = self.cf_cal(x, self.nx, self.OShift, delta,bias,fit,cf_num)
    return f
  ...
  def cf06(self, x):
    ...
    i=0
    fit[i] = self.escaffer6_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    fit[i]=10000*fit[i]/2e+7
    i=1
    fit[i] = self.schwefel_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
    i=2
    fit[i] = self.griewank_func(x,self.nx,self.OShift[i*self.nx:(i+1)*self.nx],self.M[i*self.nx:(i+1)*self.nx,0:self.nx],1,self.r_flag)
  ...
  def config_cec_functions(self, nx, func_num):
    ...
    if self.ini_flag == 0:
      ...
      # Load M matrix
      FileName = 'C:/Users/rozko/Documents/DYPLOM/ranking_algorithms/backend/algorithm_running/input_data/M_%d_D%d.txt'%(func_num, nx)
      try:
  ...
  #      print("\nError: there is insufficient memory available!\n")
      ...
      # Shift data
      FileName = "C:/Users/rozko/Documents/DYPLOM/ranking_algorithms/backend/algorithm_running/input_data/shift_data_%d.txt" %func_num
      try:
  ...
  #      print("\nError: there is insufficient memory available!\n")
      ...
      if (func_num >= 6) & (func_num <=8):
          FileName = "C:/Users/rozko/Documents/DYPLOM/ranking_algorithms/backend/algorithm_running/input_data/shuffle_data_%d_D%d.txt" %(func_num, nx)
          try:
  ...
  def call_cec22_func(self, x):    
    ...
    if self.function_to_call == 1:
      ff = self.zakharov_func(x)
      # f[i] = ff + 300.0
    elif self.function_to_call == 2:
      ff = self.rosenbrock_func(x)
      # f[i] = ff + 400.0
    elif self.function_to_call == 3:
      ff = self.schaffer_F7_func(x)
      # f[i] = ff + 600.0
    elif self.function_to_call == 4:
      ff = self.step_rastrigin_func(x)
      # f[i] = ff + 800.0
