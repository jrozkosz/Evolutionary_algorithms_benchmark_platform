function Run_EA4eig(Runs, fhd, DIM, funcs, maxFES, fistar)

Rand_Seeds=load('input_data\Rand_Seeds.txt');

Alg_Name = 'EA4eig_N100_10';
...
results = zeros(length(funcs), 6);
for func_no = funcs
    fprintf('\n-------------------------------------------------------\n')
    fprintf('Function = %d, Dimension size = %d\n', func_no, DIM)
    allerrorvals = zeros(stage + 1, Runs);

    eval(['load input_data/shift_data_' num2str(func_no) '.txt']);
    eval(['O=shift_data_' num2str(func_no) '(1,1:DIM);']);
    ...
    for run_id = 1:Runs
        ...
        rng(Rand_Seeds(mod(DIM * func_no + run_id, length(Rand_Seeds))),'twister');
        %rng(Rand_Seeds,'twister');
        ...
        run_funcvals = run_funcvals - fistar(func_no);
        %when alg. stops early, last best value is used to fill vector
        if length(run_funcvals) < stage
            run_funcvals(end + 1:stage) = bsf_fit_var - fistar(func_no);
        end
        
        fprintf('%d th run, error, success, div. = %1.8e %5.0f %4.0f \n', run_id , run_funcvals(end), succ, div_stage)
        
        if FESterm == 0
            FESterm = maxFES;
        end
        allerrorvals(:, run_id) = [run_funcvals; FESterm];
    
    end %% end 1 run
    
%     fclose(fid);
    
    allerrorvals = (sortrows(allerrorvals', 16))';
    
    results(func_no, :) = [func_no min(allerrorvals(end - 1,:)) max(allerrorvals(end - 1,:)) median(allerrorvals(end - 1,:)) mean(allerrorvals(end - 1,:)) std(allerrorvals(end - 1,:))];
    fprintf('min_max_med_mean_std:\t%e\n',results(func_no, :));
    
    
    
    file_name=sprintf('Results\\%s_%s_%s.txt',Alg_Name,int2str(func_no),int2str(DIM));
    save(file_name, 'allerrorvals', '-ascii');
    
end %% end 1 function run

file_name2 = sprintf('Results\\%s_%s_%s.txt','Basics', Alg_Name, int2str(DIM));
save(file_name2, 'results', '-ascii');
